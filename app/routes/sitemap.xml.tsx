...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const urls = [`${origin}/`, `${origin}/jobs`, ...slugs.map((slug) => `${origin}/recruitment/${encodeURIComponent(slug)}`)];
  const body = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls.map((url) => `<url><loc>${escapeXml(url)}</loc></url>`).join("")}\n</urlset>`;
